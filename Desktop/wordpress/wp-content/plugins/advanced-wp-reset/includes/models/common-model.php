...
class CommonModel {
    ...
    public function get_next_pp_infos () {
        ...
        $current_pp = $this->get_current_pp_infos();
        
        // get the pp_count 
        $max = count(AWR_PP_INFOS);

        // We increment it and reset it to 0 if past $max;
        $next_pp = ($current_pp + 1) % $max;

        // We update the option in the db
        update_option( AWR_CURRENT_PP, $next_pp );

        return $next_pp;

    }

    public function get_current_pp_infos () {
        
        // We get the value from the options, if not found, we take 0.
        $current_pp = get_option( AWR_CURRENT_PP, 0 );
        
        // get the pp_count 
        $max = count(AWR_PP_INFOS);

        // If the value is not integer or not in the range [0, $max[, we reset it at 0
        $current_pp = (int)$current_pp;
        $current_pp = $current_pp >= 0 && $current_pp < $max ? $current_pp : 0;
