<?php
/*
Plugin Name: STM Custom Icons
Plugin URI: http://stylemixthemes.com/
Description: Custom Icons plugin by StylemixThemes.
...
if ( ! class_exists( 'STM_Custom_Icons' ) ) {
	class STM_Custom_Icons {
				...
				if ( ! empty( $icon_set ) ) {
					...
					foreach ( $icon_set as $icons ) {
						foreach ( $icons as $icon ) {
							...
							$output .= '<i class="' . esc_attr( $prefix ) . esc_attr( $icon['class'] ) . '"></i><label class="icon">' . esc_html( $icon['class'] ) . '</label></li>';
						}
					}
					$output . '</ul>';
		...
		function add_zipped_font() {
            ...
            $file = json_decode( file_get_contents( $path . '/' . 'selection.json' ), true );
            ...
            $custom_json_file = $path . '/charmap.json';

            $wp_filesystem->put_contents( $custom_json_file, $json, FS_CHMOD_FILE );
...
function consulting_elementor_add_custom_icons_new( $tabs = array() ) {
    ...
    foreach ( $custom_fonts as $fontName => $info ) {
        ...
        $new_icons[ $fontName ] = [
            ...
            'ver'           => '1.0.1',
            'fetchJson'     => $path . '/stm_fonts/' . $fontName . '/charmap.json',
        ];
