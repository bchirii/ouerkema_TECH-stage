		...
		<div class="container">
			...
			<?php if ( isset( $freemius_info['plan'] ) ) : ?>
				...
				<div class="stm-type-pricing">
					<div class="stm-type-pricing-plans active" data-period="monthly"><?php esc_html_e( 'Monthly', 'aibuddy-openai-chatgpt' ); ?></div>
					<div class="stm-type-pricing-plans" data-period="annual"><?php esc_html_e( 'Annual', 'aibuddy-openai-chatgpt' ); ?></div>
					...
					<?php foreach ( $freemius_info['plan'] as $plan ) : ?>
						<div class="col-md-4"
							...
							<div class="stm_plan <?php echo esc_attr( $plan->data['classname'] ); ?>">
								...
								<div class="stm_price">
									<sup>$</sup>
									<span class="stm_price__value" data-price-monthly="<?php echo esc_attr( $plan->monthly_price ); ?>"  data-price-annual="<?php echo esc_attr( $plan->annual_price ); ?>" data-price-lifetime="<?php echo esc_attr( $plan->lifetime_price ); ?>">
										<?php echo esc_html( $plan->monthly_price ); ?>
									</span>
									<small>/<?php esc_html_e( 'per month', 'aibuddy-openai-chatgpt' ); ?></small>
								</div>
...
<script>
	jQuery(document).ready(function ($) {
		$('.stm-type-pricing-plans').on('click', function () {
			...
			let typePrice = 'monthly';
			$('.col-md-4').each(function() {
				const price = $(this).find('.stm_price__value');
				const price_label = $(this).find('.stm_price small');
				if (period === 'monthly') {
					price.text(price.attr('data-price-monthly'));
					price_label.text('/per month');
